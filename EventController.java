...
global class EventController {
    ...
    global static List<DateTime> get_checkedin_times(String event_id) {
        ...
        Map<Id, Campaign> potential_children = new Map<Id, Campaign>([SELECT Name, Description, StartDate, Status, ParentId, Id FROM Campaign WHERE ParentId=:c.id OR Id=:c.id]);
        CampaignMember[] checked_in = [SELECT LastModifiedDate, Lead.Firstname, Lead.Lastname, Lead.Email, Contact.Firstname, Contact.Lastname, Contact.Email, Contact.Company__c FROM CampaignMember WHERE Status=:Event.checkedInStatus AND CampaignId in :potential_children.keySet() ORDER BY LastModifiedDate ASC];
